...
**(Next release)** Five more themes join the lineup, and the three character themes bring their whole design language, not just a palette: **Matrix** (phosphor green terminal — CRT scanlines, text glow, sharp bloomed rectangles, all-monospace, inverted selection), **8-Bit** (console dialog frames with double borders, hard-shadow buttons that press in, square scrollbars, and pixelated map tiles), **Steampunk** (brass double-frame panels with corner rivets, engraved serif headers, and sepia-aged map tiles), plus two color moods: **Midnight** (deep blue and ice) and **Ember** (tube-amp warmth — dark browns and fire amber). All work everywhere the built-in themes do, including the map ocean color and both 2D and 3D projections.
