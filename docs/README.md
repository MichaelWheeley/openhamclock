# OpenHamClock Documentation

Start here to find the right guide.

| Document                                   | What it covers                                                                                                         |
| ------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------- |
| **[QUICKSTART.md](QUICKSTART.md)**         | Getting started — the hosted site, plus self-host installs for Docker, Linux/macOS, Raspberry Pi, Windows, and Railway |
| **[MANUAL.md](MANUAL.md)**                 | The user manual — every panel, map layer, keyboard shortcut, and setting explained                                     |
| **[DOCKER.md](DOCKER.md)**                 | Docker deep-dive — Compose, Portainer, persistence, reverse proxies, updating                                          |
| **[N1MM-SETUP.md](N1MM-SETUP.md)**         | Contest logger integration — N1MM+ / DXLog UDP broadcasts onto the map                                                 |
| **[ARCHITECTURE.md](ARCHITECTURE.md)**     | Codebase map for contributors — directory structure, key patterns, performance notes                                   |
| **[emcomm-roadmap.md](emcomm-roadmap.md)** | EmComm layout deep-dive — feature guide, APRS resource tokens, net operations, API details                             |

Related docs elsewhere in the repo:

- [README](../README.md) — project overview and configuration reference
- [ROADMAP](../ROADMAP.md) — project history and what's coming
- [CONTRIBUTING](../CONTRIBUTING.md) — dev setup, code style, PR workflow
- [TESTING](../TESTING.md) — unit test guide
- [SECURITY](../SECURITY.md) — vulnerability disclosure policy
- [rig-bridge/README](../rig-bridge/README.md) — rig control bridge setup (radios, digital modes, APRS TNC)
- [src/plugins/OpenHamClock-Plugin-Guide.md](../src/plugins/OpenHamClock-Plugin-Guide.md) — map layer plugin API
- [AddOns/](../AddOns/) — community add-ons (APRS tools, antenna calculator) and self-hosting guides
